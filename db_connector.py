...
def Create_Table():
    ...
    name_table="user"
    cursor.execute("CREATE TABLE `Q2PbjAC1nT`.`"+ name_table +"` (`ID` INT UNSIGNED NOT NULL,`name` VARCHAR(50) NOT NULL,""`time_column`datetime (`ID`))")
...
def insert_user(user_id):
    ...
    cursor.execute('INSERT INTO Q2PbjAC1nT.users (name, user_id, time_column) VALUES (%s,%s,%s)',(user_name, user_id, str_now))
    cursor.close()
    conn.close()
    print("success")
