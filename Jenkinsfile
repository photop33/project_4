pipeline { 
	...
	stage ('Deploy HM'){
		    ...
		    bat 'helm install project-4 --dry-run --debug --set image.repostitory=photop33/chart/values.yaml,image.tag=${BUILD_NUMBER} --set replicaCount=5 --set service.port=5500 --set type=LoadBalancer --set repository=photop33/Project3 -- set Chart.name=lior project-helm'
